...
st.markdown("""
<style>
    .stApp { background-color: #0b0f19; font-family: 'Inter', sans-serif; color: #c9d1d9; }
    ...
    div.stButton > button { background-color: transparent !important; border: 1px solid #30363d !important; color: #8b949e !important; width: 100%; }
...
def draw_poisson_montecarlo(lambda_expected):
    ...
    probs =[]
    for k in goals:
        p = (np.exp(-lambda_expected) * (lambda_expected**k)) / math.factorial(k)
        probs.append(p)
    ...
    fig.update_layout(
        title="Distribución de Goles Prevista (10,000 iteraciones)", 
...
def main():
    ...
    with tab1:
        ...
        with m1: 
            st.markdown("<div class='metric-item'><h2 style='color:#fff;margin:0;'>1.95</h2><div style='color:#8b949e;font-size:0.8rem;'>Cuota Mercado</div></div>", unsafe_allow_html=True)
        with m2: 
            st.markdown("<div class='metric-item'><h2 style='color:#8b949e;margin:0;'>51.2%</h2><div style='color:#8b949e;font-size:0.8rem;'>Prob. Implícita</div></div>", unsafe_allow_html=True)
        with m3: 
            st.markdown("<div class='metric-item' style='border-color:#3fb950;'><h2 style='color:#3fb950;margin:0;'>+13.2%</h2><div style='color:#8b949e;font-size:0.8rem;'>Edge (+EV)</div></div>", unsafe_allow_html=True)
        with m4: 
            st.markdown("<div class='metric-item'><h2 style='color:#f0883e;margin:0;'>2.5 U</h2><div style='color:#8b949e;font-size:0.8rem;'>Bankroll (Kelly)</div></div>", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if st.button("EJECUTAR SIMULACIÓN MATEMÁTICA"):
            with st.spinner("Procesando matriz histórica y simulando colisiones estocásticas..."):
                ...
                with c1: 
                    st.markdown(f"<div class='metric-item'><h2 style='color:#58a6ff;margin:0;'>{prob_local}%</h2><div style='color:#8b949e;'>Probabilidad LOCAL</div></div>", unsafe_allow_html=True)
                with c2: 
                    st.markdown(f"<div class='metric-item'><h2 style='color:#8b949e;margin:0;'>{prob_emp}%</h2><div style='color:#8b949e;'>Probabilidad EMPATE</div></div>", unsafe_allow_html=True)
                with c3: 
                    st.markdown(f"<div class='metric-item'><h2 style='color:#f85149;margin:0;'>{prob_vis}%</h2><div style='color:#8b949e;'>Probabilidad VISITANTE</div></div>", unsafe_allow_html=True)
                
                fig_1x2 = go.Figure(data=[go.Bar(
                    x=['LOCAL', 'EMPATE', 'VISITANTE'], 
                    y=[prob_local, prob_emp, prob_vis], 
                    marker_color=['#58a6ff', '#30363d', '#f85149'], 
                    text=[f"{prob_local}%", f"{prob_emp}%", f"{prob_vis}%"], 
                    textposition='auto'
                )])

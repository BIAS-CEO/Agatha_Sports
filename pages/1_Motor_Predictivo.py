...
if 'fixtures' in st.session_state and not st.session_state['fixtures'].empty:
    ...
    with col_l:
        target_league = st.selectbox("FILTRAR POR COMPETICIÓN",["TODAS"] + ligas_disponibles)
    ...
    if st.button("EJECUTAR ANÁLISIS DE VECTOR MATEMÁTICO (DOV)"):
        ...
        with st.spinner("Ingestando telemetría y procesando inferencia LLM..."):
            ...
            odds_dict = df_odds.to_dict('records') if not df_odds.empty else {"warning": "Sin cuotas en The Odds API."}
            ...
            if "error" in resultado_ia:
                ...
                st.markdown("<h3 style='color:#88C0D0;'>REPORTE DE INTELIGENCIA MATEMÁTICA</h3>", unsafe_allow_html=True)
                prob = resultado_ia.get("evaluacion_probabilidades", {})
                mercado = resultado_ia.get("analisis_mercado", {})
                
                r1, r2, r3 = st.columns(3)
                with r1:
                    st.markdown("<div class='metric-box'><h4>PROBABILIDAD REAL 1X2</h4>", unsafe_allow_html=True)
                    st.write(f"**LOCAL:** {prob.get('1X2', {}).get('local', 0)}%")
                    st.write(f"**EMPATE:** {prob.get('1X2', {}).get('empate', 0)}%")
                    st.write(f"**VISITANTE:** {prob.get('1X2', {}).get('visitante', 0)}%")
                    st.markdown("</div>", unsafe_allow_html=True)
                    
                with r2:
                    is_value = mercado.get('hay_value_bet', False)
                    box_class = 'metric-box success-box' if is_value else 'metric-box warning-box'
                    st.markdown(f"<div class='{box_class}'><h4>ESTADO DEL MERCADO</h4>", unsafe_allow_html=True)
                    st.write(f"**VALUE BET:** {'AFIRMATIVO' if is_value else 'NEGATIVO'}")
                    st.write(f"**MOVIMIENTO:** {mercado.get('analisis_movimiento', 'N/A')}")
                    st.markdown("</div>", unsafe_allow_html=True)
                ...
                with r3:
                    ...
                    st.markdown(f"<div class='{risk_class}'><h4>DIRECTIVA RIESGO</h4>", unsafe_allow_html=True)
                    st.write(f"**NIVEL:** {risk}")
                    st.write(f"**ESTRATEGIA:** {bankroll.get('estrategia', 'N/A')}")
                    st.markdown("</div>", unsafe_allow_html=True)
                
                st.markdown(f"**JUSTIFICACIÓN:** *{prob.get('justificacion_matematica', 'N/A')}*")
                
                st.markdown("<h3 style='color:#88C0D0;'>TARGETS DE INVERSIÓN (TOP 3)</h3>", unsafe_allow_html=True)
                ...
                if top_preds:
                    for i, pred in enumerate(top_preds):
                        ...
                        <div style="background-color:#0D1117; border: 1px solid #30363D; padding: 10px; margin-bottom: 5px;">
                            ...
                            <span style="color:#EBCB8B;">EDGE: {pred.get('edge_estimado_pct')}%</span><br>
                            <span style="color:#8B949E; font-size: 0.9rem;">>> TÁCTICA: {pred.get('razonamiento')}</span>
                        </div>
                        """, unsafe_allow_html=True)
                else:
                    st.info("Sin objetivos viables para este nodo.")
                
                with st.expander("VER DOSSIER RAW (JSON)"):
